...
**功能**：
- macOS 菜单栏显示图标 + 可配置的数据维度（默认显示总 Token 消耗量）
- 支持 6 种显示维度：总计 Token / 今日 Token / 总计成本 / 今日成本 / 总计请求数 / 今日请求数
- 用户可在"设置 → 菜单栏显示"中自定义默认显示维度，切换后立即生效
- 左键点击弹出原生上下文菜单：今日统计 / 总计统计 / 刷新 / 打开窗口 / 退出
- 关闭主窗口时隐藏到后台（不退出），通过菜单栏可重新打开
- 每 60 秒自动刷新菜单栏数据

**配置存储**：
- SQLite `settings` 表（key-value），存储 `tray_display_metric` 键
- 通过 Tauri 命令 `get_tray_display_metric` / `set_tray_display_metric` 读写
- `set_tray_display_metric` 保存后立即调用 `update_tray_display` 更新菜单栏
...
| Commit | 说明 |
|--------|------|
| (pending) | feat: tray display config — configurable metric + settings table + CSP fix |
| `3ae560b` | feat: macOS menu bar (system tray) — show token cost in menu bar |
