...
### 🔴 高优先级：桌面小组件多项功能未工作（2026-04-30 分析完成，待修复）

**问题清单与根因分析**：

#### 1. 悬浮窗不能拖拽
- **根因**：Tauri v2 的 `data-tauri-drag-region` 只对被直接点击的元素生效，不传播到子元素。header div 内部被子元素完全填满，用户点击到的永远是子元素而非 div 本身，因此拖拽永远不触发。参考：https://v2.tauri.app/learn/window-customization/
- **修复方案**：改用编程式 `getCurrentWindow().startDragging()` + `onMouseDown` handler，按钮用 `e.stopPropagation()` 阻止冒泡

#### 2. 悬浮窗关闭按钮不工作
- **根因**：权限已添加但可能需要清理 `src-tauri/gen/` 目录重新生成 capabilities
- **修复方案**：添加 `core:window:allow-close` 权限，构建前 `rm -rf src-tauri/gen/`

#### 3. 设置页小组件配置卡片不显示
- **根因**：代码结构正确，最可能是 Vite 构建缓存问题
- **修复方案**：清理 `node_modules/.vite` 和 `src-tauri/gen/`，重新验证

#### 4. 位置持久化性能问题
- **根因**：`WindowEvent::Moved` 每次触发都读写磁盘，拖拽时产生大量 I/O
- **修复方案**：在 `widget.rs` 中添加 500ms debounce

#### 5. identifier 数据丢失风险
- **根因**：`tauri.conf.json` 中 identifier 从 `com.asus.token-cost-analyzer` 改为 `com.token-cost-analyzer.app`，导致数据目录变更
- **修复方案**：恢复原 identifier `com.asus.token-cost-analyzer`

**详细修复方案**：见 `.claude/projects/.../memory/project_widget_fix_plan.md`
**执行顺序**：P5 → P1 → P2 → P4 → 清理缓存 → 验证
